...
public class PCBonds implements Serializable {
  ...
  public static class Order implements Serializable {
    ...
    public int decode(final InputStream is, final boolean withTag) throws IOException {
      ...
      if (length.val == -1) {
        while (true) {
          ...
          PCBondType element = new PCBondType();
          subCodeLength += element.decode(is, false);
          seqOf.add(element);
        }
      }
    ...
    public void appendAsString(final StringBuilder sb, final int indentLevel) {
      ...
      if (seqOf == null) {
        ...
        Iterator<PCBondType> it = seqOf.iterator();
        if (it.hasNext()) {
          sb.append(it.next());
          while (it.hasNext()) {
            sb.append(",\n");
  ...
  public int decode(final InputStream is, final boolean withTag) throws IOException {
    ...
    if (totalLength == -1) {
      ...
      if (berTag.equals(BerTag.CONTEXT_CLASS, BerTag.CONSTRUCTED, 2)) {
        codeLength += length.decode(is);
        order = new Order();
        subCodeLength += order.decode(is, true);
        subCodeLength += berTag.decode(is);
        if (length.val == -1) {
          is.read();
          berTag.decode(is);
  ...
  public void appendAsString(final StringBuilder sb, final int indentLevel) {
    ...
    if (order != null) {
      sb.append("order: ");
      order.appendAsString(sb, indentLevel + 1);
    } else {
      sb.append("order: <empty-required-field>");
    }
